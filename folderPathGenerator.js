export function generateFolderPath(rootDirectory) {
            ...
            chrome.scripting.executeScript(
                ...
                (results) => {
                    ...
                    if (result && result.found) {
                        // Prepend root directory to the path
                        const fullFolderPath = `${rootDirectory}/${result.path}`;
                        resolve(fullFolderPath);
                    } else {
                        reject(result.path || 'An error occurred while generating the path.');
                    }
                }
            );
        });
